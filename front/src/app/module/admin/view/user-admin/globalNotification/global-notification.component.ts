...
@Component({
    ...
    template: `
        ...
        <div class="modal-footer">
            ...
            <button type="button" class="btn btn-outline-primary" (click)="send(selectedNotification.message,selectedNotification.status)">Envoyer</button>
        </div>
    `,
...
export class GlobalNotificationComponent implements OnInit {
    ...
    send(message: string, status: string) {
      this.notificationService.saveToEveryOne(message,status).subscribe(
          data => {
            console.log(data);
            this.selectedNotification = new Notification();
